...
function formatDate(d) {
    return d.getFullYear()+"-0"+(d.getMonth()+1)+"-0"+d.getDay()+"T"+d.getHours()+":"+d.getMinutes()+":00"
}

ErsteBankClient.prototype.getDataExport = function() {
    var self = this;
    return self.getAccountId()
        .then(function(accountId) {
            const strf_format = '%Y-%m-%dT%H:%M:%S'
